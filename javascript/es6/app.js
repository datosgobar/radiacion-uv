...
const getWeather = (updatingFunction) => {
  ...
  $.get(url, (data) => {
    ...
    // Radiation UV
    let radiation = parseFloat(data.current_observation.UV);
    if (radiation < 0) { radiation = 1; } // Entorno de desarrollo
    // Temperature
...
$(document).ready(() => {
        ...
        averageArray.forEach((v, k) => {
          ...
          if (average(averageArray[k]) !== 0) {
            ...
            $('#forecast').append(`<article>
              ...
              <span class="pronostico">Sin radiación</span>
            </article>`);
          }
  ...
  let section2 = $('#section2').children().first();
  let section2Esq;
  ...
  const createLineChart = (data) => {
    ...
    // Set the ranges
    let x = d3.scaleTime().domain([new Date(2017, 0, 3), new Date(2017, 0, 10)]).range([0, width]);
  ...
  setInterval(() => { clock(new Date(), '#time'); }, 1000);
});
