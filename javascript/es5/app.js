...
$(document).ready(function () {
  ...
  var measureRadiation = function measureRadiation() {
    var inicial = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';


    var radiationVal = inicial === 'inicial' ? 11 : scaleRadiation(radiation.now.number);
  ...
  var stateRadiation = function stateRadiation(radiationLvl, data) {
  ...
  var start = function start() {
    ...
    var getWeather = function getWeather() {
      ...
      var url = 'http://dataservice.accuweather.com/currentconditions/v1/7893.json?apikey=' + config.apiKey + '&language=' + config.lenguage + '&details=' + config.details;

      $.get(url, function (data) {
        xmlToJson(data).report.location.day.forEach(function (v, k) {
          var date = new Date();
  ...
  var generateRadiation = function generateRadiation() {
    ...
    var valueRadiation = Math.random() * 15 + 1;
    var formatValue = valueRadiation.toString().split('.');
  ...
  var measure = measureRadiation('inicial');
  var measureUv = measureRadiation();

  var svgContainer = d3.select('#medidor').append('svg').attr('width', radiation.diameter).attr('height', function () {
  ...
  .attr('id', 'clock').attr('text-anchor', 'middle').attr('alignment-baseline', 'central').style('fill', 'white').style('font-weight', 100).style('font-size', '1.75rem').style('transform', function () {
    return 'translate(' + radiation.diameter / 2 + 'px, ' + radiation.diameter / 4 + 'px)';
  });
  svgContainer.append('text') // Nivel de Radiacion UV
  .attr('id', 'lvlRadiation').attr('text-anchor', 'middle').attr('alignment-baseline', 'end').style('fill', 'white').style('font-size', '15rem');
  svgContainer.append('text') // Nivel de Radiacion UV (decimales)
  .attr('id', 'lvlRadiationD').attr('text-anchor', 'middle').attr('alignment-baseline', 'end').style('fill', 'white').style('font-size', '3rem');
