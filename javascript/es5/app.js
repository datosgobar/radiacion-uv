...
var getWeather = function getWeather(updatingFunction) {
  ...
  $.get(url, function (data) {
    ...
    // Radiation UV
    var radiation = parseFloat(data.current_observation.UV);
    if (radiation < 0) {
      radiation = 1;
    ...
    updatingFunction(radiation, temp, icon);
  });
};
...
$(document).ready(function () {
  ...
  var fullpage = {
    ...
    slidesNavigation: $(window).outerWidth() > 600,
    ...
    slideSelector: $(window).outerWidth() > 600 ? '' : '.slide',
    lazyLoading: true,
  ...
  var updating = function updating(uv, temp, icon) {
    console.log(radiation);
    // Reset Timer Update
    secondCount = 0;

    // Update UV
    radiation.now = uv; // Update Radiation Object
  ...
  var start = function start() {
    // Forecast Control
    var forecastCalculate = function forecastCalculate() {
      var now = new Date();
      var averageArray = [];
      ...
      $.get('http://geouv.citedef.gob.ar/api/uvDiario/581b4637445ea55b1b1769f3', function (data) {

        data.medidas.forEach(function (v, k) {
        ...
        averageArray.forEach(function (v, k) {
          ...
          if (average(averageArray[k]) !== 0) {
            $('#forecast').append('<article>\n              <span class="pronostico_horario">' + formatDate(k) + ':00</span>\n              <svg class="pronostico_barra" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 235 15">\n                <line fill="none" stroke="silver" stroke-linecap="round" stroke-linejoin="round" stroke-width="15px" x1="7.5" y1="7.5" x2="227.5" y2="7.5"/>\n                <line fill="none" stroke="' + stateRadiation(radiation, average(averageArray[k]), 'color') + '" stroke-linecap="round" stroke-linejoin="round" stroke-width="15px" x1="7.5" y1="7.5" x2="' + 227.5 / sizeObject(radiation.states) * (stateRadiation(radiation, average(averageArray[k]), 'position') + 1) + '" y2="7.5"/>\n                <circle fill="white" stroke="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="15px" cx="' + 227.5 / sizeObject(radiation.states) * (stateRadiation(radiation, average(averageArray[k]), 'position') + 1) + '" cy="7.44" r="5.15"/>\n              </svg>\n              <span class="pronostico_estado">' + stateRadiation(radiation, average(averageArray[k]), 'name') + '</span>\n            </article>');
          } else {
            $('#forecast').append('<article>\n              <svg height="50" width="50"><circle cx="25" cy="25" r="20" fill="white" stroke="silver" stroke-width="2" stroke-dasharray="9" stroke-linejoin="round" /></svg>\n              <span class="horario">' + formatDate(k) + ':00</span>\n              <span class="pronostico">Sin radiaci\xF3n</span>\n            </article>');
          }
        });
      }).fail(function () {
        console.error('No hay pronostico');
        $('#forecast').append('no hay pronostico');
      });
    };
    // Protection UV Control
    var protectionCalculate = function protectionCalculate() {
  ...
  var section2 = $('#section2').children().first();
  var section2Esq = void 0;

  if ($(window).outerWidth() > 600) {
    $.fn.fullpage.destroy('all');
    $('#sectionDer').append(section2.children());
    section2Esq = section2;
    section2.remove();
    $('#fullpage').fullpage(fullpage);
  }
  ...
  $(window).resize(function () {
    ...
    if (sizeWindow > 600) {
      $.fn.fullpage.destroy('all');
      section1.append(section2.children());
      section2Esq = section2;
      section2.remove();
      $('#fullpage').fullpage(fullpage);
    } else {
      $.fn.fullpage.destroy('all');
      section2Esq.insertAfter('#section1');
      section2.append(section1.children().last());
      $('#fullpage').fullpage(fullpage);
    }
  ...
  var createLineChart = function createLineChart(data) {
    ...
    // Set the ranges
    var x = d3.scaleTime().domain([new Date(2017, 0, 3), new Date(2017, 0, 10)]).range([0, width]);
    ...
    graph.append('svg:path').attr('id', 'lineChartGraph').attr('d', line(dataset)).attr('stroke', 'url(#image)').attr('stroke-linejoin', 'round').attr('stroke-width', '0.5rem');
  };
  ...
  // const refreshLastUpdate = (minutes) => {
  //   $('#actualizacion').text(`ÚLTIMA ACTUALIZACION HACE ${ minutes } MINUTOS`);
  //
  //   if (minutes === 0) {
  //     resetLastUpdate();
  //   }
  ...
  // Intervals
  // setInterval(getWeather, (1000 * 60 * 60)); // Se consulta el clima actual cada 1 hora.
  // setInterval(getWeather, (1000 * 60 * 60)); // Se consulta pronostico UV cada 1 hora.
